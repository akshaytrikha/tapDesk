...
# TODO: part c.3: modify function signature to include preprocessors
def make_pipeline(classifier, preprocessor_list, n, d):
    """Make ML pipeline.

    Parameters
    --------------------
        classifier        -- classifier, string
        preprocessor_list -- preprocessor, list
    ...
    for preprocessor in preprocessor_list:
        preprcsr = getattr(preprocessors, preprocessor)(n,d)
        name =  type(preprcsr).__name__
        transformer = preprcsr.transformer_
        steps.append((name, transformer))
        for key, val in preprcsr.param_grid_.items():
            param_grid[name + "__" + key] = val

    ### ========== TODO : END ========== ###

    # get classifier and hyperparameters to tune using cross-validation
    clf = getattr(classifiers, classifier)(n,d) #initialize classifier
...
def run(dataset, preprocessor_list, classifier):
    ...
    --------------------
        ...
        preprocessor_list -- preprocessors, list
        classifier        -- classifier, string
    """
    ...
    ### ========== TODO : START ========== ###
    # part c.3: modify function call to use preprocessors

    # make pipeline
    pipe, param_grid = make_pipeline(classifier, preprocessor_list, n, d)
    ...
    # save to file
    ### ========== TODO : START ========== ###
    # part c.3: modify prefix to include preprocessors
    preprcs = ''
    for i in range(len(preprocessor_list)):
        if i != len(preprocessor_list) - 1:
            preprcs += preprocessor_list[i] + '_'
        else:
            preprcs += preprocessor_list[i]

    prefix = os.path.join("results", '_'.join([dataset] + [preprcs] +[classifier]))
    print(prefix)
    ### ========== TODO : END ========== ###
